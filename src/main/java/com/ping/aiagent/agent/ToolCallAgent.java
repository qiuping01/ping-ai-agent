...
public class ToolCallAgent extends ReActAgent {
    ...
    public ToolCallAgent(ToolCallback[] availableTools) {
        ...
        // 核心代码 - 禁用Spring AI 内置的工具调用机制，自己维护选项和消息上下文
     ...
     * 处理当前状态并决定下一步行动
     *
     * @return 是否需要执行行动，true表示需要调用工具，false表示无需行动
     */
    @Override
    public boolean think() {
        // 步骤1：准备AI调用 - 添加下一步提示词到消息上下文
        ...
        try {
            ...
            // 步骤3：分析AI响应，判断是否需要使用工具
            AssistantMessage assistantMessage = chatResponse.getResult().getOutput();
            List<AssistantMessage.ToolCall> toolCallList = assistantMessage.getToolCalls();

            // 记录AI的思考过程和工具选择
            String thinkingResult = assistantMessage.getText();
            log.info("{} 的思考：{}", getName(), thinkingResult);
            ...
            // 根据是否有工具调用来决定返回值和消息处理
            if (toolCallList.isEmpty()) {
                // 无工具调用时，手动添加助手消息到上下文
                getMessageList().add(assistantMessage);
                return false;
            } else {
                // 有工具调用时，act方法执行时会自动处理消息上下文
                return true;
            }
        } catch (Exception e) {
            log.error("{} 思考过程异常：{}", getName(), e.getMessage());
            // 将错误信息添加到消息上下文，供后续处理参考
